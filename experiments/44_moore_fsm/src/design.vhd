...
entity moore_1011 is port(clk, rst_n, din : in std_logic; detected : out std_logic); end entity;
architecture rtl of moore_1011 is
...
begin
  ...
  process(state, din) begin
    case state is
      ...
      when s101 => if din='1' then next_state <= s1011; else next_state <= s10; end if;
      when others => if din='1' then next_state <= s1; else next_state <= s10; end if;
    end case;
  end process;
